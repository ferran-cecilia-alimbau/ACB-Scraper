...
def load_game_info() -> pd.DataFrame:
    ...
    jornada_text = df["jornada"].astype("string")
    jornada_num = jornada_text.str.extract(r"(\d+)", expand=False)
    df["jornada_num"] = pd.to_numeric(jornada_num, errors="coerce").fillna(0).astype(int)
