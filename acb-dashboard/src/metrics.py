...
def estimate_possessions(fga: float, fta: float, tov: float, oreb: float) -> float:
    """Posesiones estimadas = FGA + 0.44 * FTA + TOV - OREB"""
...
def usage_rate(fga: float, fta: float, tov: float, player_min: float,
    ...
    """USG% = 100 * ((FGA + 0.44 * FTA + TOV) * (Team_Min / 5)) / (Player_Min * (Team_FGA + 0.44 * Team_FTA + Team_TOV))"""
...
def four_factors(team_row: dict) -> dict:
    """Calcula los Four Factors de Dean Oliver para un equipo.

    Returns:
        Dict con eFG%, TOV%, OREB%, FT_rate
    """
...
def add_advanced_stats_to_players(player_stats: pd.DataFrame) -> pd.DataFrame:
    """Añade métricas avanzadas al DataFrame de jugadores."""
    df = player_stats.copy()

    # FGA total = T2 intentados + T3 intentados
    ...
    for col in per36_cols:
        df[f"{col}_per36"] = df.apply(
            lambda r: per_36(r[col], r["minutos_decimal"]), axis=1
        )
...
def add_advanced_stats_to_teams(team_stats: pd.DataFrame, game_info: pd.DataFrame) -> pd.DataFrame:
    """Añade métricas avanzadas al DataFrame de equipos por partido."""
    df = team_stats.copy()
    ...
    # Calcular DRtg: necesitamos puntos del oponente
    # Merge con game_info para obtener el oponente
    ...
    merged["pace"] = merged.apply(
        lambda r: pace(r["est_possessions"], 40), axis=1  # 200 min equipo / 5 jugadores
    )
